...
glb.set_value("MYSQL_IP", 'localhost')
...
class ChattingWin(QMainWindow,Ui_chattingWin):
    ...
    def Open(self,patient_id):
        ...
        self.show()
        print("main_open 函数",self.newflag)
        if(self.newflag!=1):
            self.patient_id = patient_id
            print(self.patient_id)
        else:
            self.patient_id=self.newpatient_id
            print("self.newpatient_id",self.newpatient_id)
        ...
        print("self.patient_id", self.patient_id)
...
# 登录界面
    ...
    main.pushButton_2.clicked.connect(reg.show)

    #注册界面
    reg.pushButton.clicked.connect(reg.reg)
    reg.pushButton_2.clicked.connect(reg.close)

    # 点击有人咨询
    doctor.pushButton.clicked.connect(lambda:chattingWin.Check(doctor.newflag))
    # 这句话有问题########################################
    doctor.pushButton.clicked.connect(lambda:chattingWin.Open(doctor.newflag))
